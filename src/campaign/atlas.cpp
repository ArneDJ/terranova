...
// TODO save the drainage basin map and do relief altering somewhere else
...
void Atlas::assign_mountain_ridges()
{
	...
	const uint8_t color_high = 255;
	const uint8_t color_low = 200;

	// normal ridge cases
	// based on the drainage basin we will asign mountain ridges
	...
	// special case where the mountain rage is thin
	// these ranges only have a "width" of one tile so we will draw our ridge through them
	for (const auto &border : m_borders) {
		const auto &edge = m_graph.edges[border.index];
		auto &left = m_tiles[edge.left_cell->index];
		auto &right = m_tiles[edge.right_cell->index];
	...
	for (int x = 0; x < m_heightmap.width(); x++) {
		for (int y = 0; y < m_heightmap.height(); y++) {
			...
			if (amp) {
				...
				m_heightmap.plot(x, y, util::CHANNEL_RED, height + 0.2f * noise);
			}
		}
	}
}
